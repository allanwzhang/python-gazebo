...
namespace gazebo {
	...
	class S1000_Motor_Model : public ModelPlugin {
		...
		public: void OnUpdate() {
			...
			//		* Apply Forces and Torques to the CoG of the frame link 
		...
		public: void ApplyForcesAndTorques() {
			...
			//Create Column Vector by multiply R_b_e_tr by column vector
			double Ftot_b[3];
			...
			Mx = l * (T[0] + (sqrt(2)/2) * T[1] + (sqrt(2)/2) * T[7]
				- T[4] - (sqrt(2)/2) * T[3] - (sqrt(2)/2) * T[5]);
				
			// Roll (My) Left and Right Movement
			My = l * (T[6] + (sqrt(2)/2) * T[5] + (sqrt(2)/2) * T[7]
				- T[2] - (sqrt(2)/2) * T[1] - (sqrt(2)/2) * T[3]);
